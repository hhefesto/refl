...
hints:
- hidden: false
  text: Computation can establish equality even under nested operations and variables.
- hidden: true
  text: Normalize each side. Here addition recurses on the right numeral, leaving
    successors around `x`.
- hidden: true
  text: Try Refine on an equality constructor after comparing those normal forms.
learning_goals:
- '**Refine** (`C-c C-r`) puts an expression in the hole and opens new holes for its
  missing arguments.'
- '**Normalise** (`C-c C-n`) shows what an expression computes to.'
- '`_+_` computes on its second argument, so `(x + 2) + 1` and `x + 3` are the same
  by definition.'
title: Refine and Normalise
---
Computation can establish equality even under nested operations and variables.

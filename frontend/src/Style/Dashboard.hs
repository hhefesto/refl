...
dashboardCss = T.unlines
  ...
  , ".dash, .dash * { box-sizing:border-box; } .dash .panel { min-width:0; }"
  , ".daily-values { overflow-x:auto; } .daily-values th, .daily-values td { padding:.3rem; text-align:left; }"
  ...
  -- Refresh, and the Retry an error offers, belong in the header row: loose
  -- above the tiles they read as part of the report rather than its controls.
  , ".dash-top .acts { display:flex; gap:.3rem; }"
  , ".coverage { color:var(--muted); font-size:.8rem; }"
  ...
  , ".panels { display:grid; grid-template-columns:repeat(auto-fit, minmax(min(100%, 380px), 1fr)); gap:.8rem; align-items:start; }"
  ...
  , ".chart .concurrent { fill:none; stroke:var(--cat-2); stroke-width:2; stroke-dasharray:6 4; }"
  ...
  , ".bars .row { display:grid; grid-template-columns:minmax(0, 38%) minmax(0, 1fr) auto; gap:.5rem; align-items:center; font-size:.85rem; }"

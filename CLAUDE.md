...
- `backend` — `Refl.Language` is the plugin record; `Refl.Language.Agda`
  ...
  one prover per connection; `Refl.Server.Analytics` records visits;
  `Refl.Check` is the content CI and the world module generator; `backend/browser` is the headless-Chromium acceptance
...
- `frontend` — `App` (router shell), `Client` (XHR + websocket),
  ...
  `Dashboard` (+ its own `Style.Dashboard`); `Widgets.InputTable` is
  generated, and so is `world-countries.json` (`refl-gen-world-map`).
...
- Analytics stores no IP address: the peer is resolved to a country on
  arrival and dropped, and the only per-person key is the anonymous
...
- Two traps the analytics code exists to avoid, both found by running it:
  ...
  and `anLock` orders reads against writes — otherwise the dashboard reads
  zero for today, every day. And `ss404Handler` answers *any* unknown path
...
- nginx inherits `proxy_set_header` as a whole array: a location that
  ...
  must be set **inside** `locations."/"`, not at server level, or X-Real-IP
  silently vanishes. `checks.ingress` asserts this by evaluation; it fails
  if either that or the Cloudflare `set_real_ip_from` block is removed.
...
- The dashboard palettes come from the dataviz skill's validator, not from
  taste: categorical ΔE 13.8 under colour blindness, sequential one hue
  with its lowest step at 2.12:1 against the card. They live in
  `Style.Dashboard`, injected by that page, because `Style.appCss` is
  inlined into every public page load.
- Natural Earth 1:110m has no outline for the microstates (Singapore,
  Malta, Monaco), so the table beside the map names any country it cannot
  draw. Country names come from the browser's `Intl.DisplayNames`, so no
  name table ships.
...
- After changing the donation addresses or the country outlines: the
  address checksums are re-derived by `checks.donations`, and
  `world-countries.json` is regenerated by hand with `refl-gen-world-map`
  (see its header) — neither is rebuilt by CI.
